...
impl Interface {
    ...
    pub async fn launch<E>(worker_uri: &str, board: Board, options: Options, evaluator: E) -> Self
    where
        ...
        E::Reward: Serialize + DeserializeOwned,
    {
        ...
        let worker = Worker::new(
            worker_uri,
            bot_thread,
            &(board, options, evaluator, worker_uri.to_owned()),
        )
        .await
        .unwrap();
...
fn bot_thread<E>(
    (board, options, eval, worker_uri): (Board, Options, E, String),
